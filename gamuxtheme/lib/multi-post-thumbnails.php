	...
	class MultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.8', '<')) {	
				...
				wp_enqueue_script("mpt-featured-image",  $template_url, array('jquery'));
			} else { // 3.8+ media modal
				wp_enqueue_media();
				$template_url_1 = get_bloginfo('template_url') . '/js/multi-post-thumbnails-admin.js';
				wp_enqueue_script("mpt-featured-image",  $template_url_1, array('jquery'));
				$template_url_2 = get_bloginfo('template_url') . '/js/media-modal.js';
				wp_enqueue_script("mpt-featured-image-modal",  $template_url_2, array('jquery'));				
			}
		}
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			if (version_compare($wp_version, '3.8', '<')) {
				// Use the old thickbox for versions prior to 3.8
				...
				// Use the media modal for 3.8 and up
				$image_library_url = "#";
				$modal_js = sprintf(
